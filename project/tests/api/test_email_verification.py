...
class TestEmailVerification(TestCase):
    ...
    def test_verification(self):
        res = self.client.get(reverse(self.verify_email_url, kwargs={"encoded_email": self.encoded_email}))
        self.assertEqual(res.status_code, status.HTTP_200_OK)

    def test_verification_with_unauthorized_email(self):
        encoded_email = 'ZHVtZHVtQG1haWwuY29t'
        res = self.client.get(reverse(self.verify_email_url, kwargs={"encoded_email": encoded_email}))
